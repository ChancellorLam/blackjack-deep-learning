...
class BlackjackGame:
    ...
    def get_player_decision(self, player_num):
        prompt_with_decisions = self.construct_prompt_based_on_game_state(player_num)
    ...
    def construct_prompt_based_on_game_state(self, player_num):
        prompt = "Would you like to Hit"
        decisions = ["Stand"]
        if self.turn == 0:
            decisions.append("Double")
            if self.player_hands["Player " + str(player_num)][0].can_split():
